...
public class FirstPersonController : MonoBehaviour
{
    ...
    public bool useRightClickToZoom = true; // Added to replace KeyCode.Mouse1 default
    ...
    // public Key sprintKey = Key.LeftShift; // Replaced by InputActionReference
    ...
    // Sprint Bar (Legacy - keeping for compatibility but prioritizing HUDManager)
    public bool useSprintBar = false; // Default to false to use HUDManager
    ...
    // public Key jumpKey = Key.Space; // Replaced by InputActionReference
    ...
    // public Key crouchKey = Key.LeftCtrl; // Replaced by InputActionReference
    ...
    private void Update()
    {
        ...
        if(cameraCanMove && lookAction != null)
        {
            Vector2 mouseDelta = lookAction.action.ReadValue<Vector2>() * 0.1f; 
        ...
        if (enableZoom && zoomAction != null)
        {
            ...
            bool zoomPressed = zoomAction.action.WasPressedThisFrame() || (useRightClickToZoom && WasRightClickPressed());
            bool zoomReleased = zoomAction.action.WasReleasedThisFrame() || (useRightClickToZoom && WasRightClickReleased());
            
            if(zoomPressed && !holdToZoom && !isSprinting)
            {
                // Toggle mode
                isZoomed = !isZoomed;
            }
        ...
        if(enableSprint)
        {
            ...
            // Update HUDManager Stamina
            if (HUDManager.Instance != null && !unlimitedSprint)
            {
                HUDManager.Instance.UpdateStamina(sprintRemaining / sprintDuration);
            }
        }
        ...
        if(enableJump && jumpAction != null && jumpAction.action.WasPressedThisFrame() && isGrounded)
        {
            Jump();
        }
        ...
        {
            if(crouchAction.action.WasPressedThisFrame() && !holdToCrouch)
            {
                Crouch();
            }
            ...
            else if(crouchAction.action.WasReleasedThisFrame() && holdToCrouch)
            {
                isCrouched = true;
                Crouch();
            }
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove && moveAction != null)
        {
            ...
            bool isSprintingInput = sprintAction != null && sprintAction.action.IsPressed();
    ...
    // Input Helpers (Removed legacy polling, now using InputActionReference)
    private bool WasRightClickPressed()
    {
        return Mouse.current != null && Mouse.current.rightButton.wasPressedThisFrame;
    }

    private bool WasRightClickReleased()
    {
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        // Updated to use InputActionReference
        fpc.zoomAction = (InputActionReference)EditorGUILayout.ObjectField(new GUIContent("Zoom Action", "Determines what input action is used to zoom."), fpc.zoomAction, typeof(InputActionReference), false);
        fpc.useRightClickToZoom = EditorGUILayout.ToggleLeft(new GUIContent("Use Right Click to Zoom", "If enabled, right mouse click will also trigger zoom."), fpc.useRightClickToZoom);
        ...
        fpc.moveAction = (InputActionReference)EditorGUILayout.ObjectField(new GUIContent("Move Action", "Input action for movement (Vector2)."), fpc.moveAction, typeof(InputActionReference), false);
        fpc.lookAction = (InputActionReference)EditorGUILayout.ObjectField(new GUIContent("Look Action", "Input action for looking (Vector2)."), fpc.lookAction, typeof(InputActionReference), false);
        ...
        // Updated to InputActionReference
        fpc.sprintAction = (InputActionReference)EditorGUILayout.ObjectField(new GUIContent("Sprint Action", "Determines what input action is used to sprint."), fpc.sprintAction, typeof(InputActionReference), false);
        ...
        fpc.jumpAction = (InputActionReference)EditorGUILayout.ObjectField(new GUIContent("Jump Action", "Determines what input action is used to jump."), fpc.jumpAction, typeof(InputActionReference), false);
        ...
        fpc.crouchAction = (InputActionReference)EditorGUILayout.ObjectField(new GUIContent("Crouch Action", "Determines what input action is used to crouch."), fpc.crouchAction, typeof(InputActionReference), false);
